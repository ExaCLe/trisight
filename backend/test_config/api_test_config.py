...
from backend.test_config.crud_test_config import create_test_config, get_all_test_configs, delete_test_config, \
    update_test_config, \
...
def create_test_config_endpoint(test_config: schemas.TestConfig, db: Session = Depends(get_db)):
    return create_test_config(db=db, test_config=test_config)
...
def read_test_config_endpoint(test_config_id: int, db: Session = Depends(get_db)):
    ...
    if test_config is None:
        raise HTTPException(status_code=404, detail=f"Test Config with id {test_config_id} not found")
    return test_config


# Endpoint to delete a test config by id
@router.delete("/{test_config_id}", response_model=schemas.TestConfigResponse)
def delete_test_config_endpoint(test_config_id: int, db: Session = Depends(get_db)):
    test_config = delete_test_config(db, test_config_id=test_config_id)
    if test_config is None:
        raise HTTPException(status_code=404, detail=f"Test Config with id {test_config_id} not found")
    return test_config
...
def update_test_config_endpoint(test_config_id: int, test_config: schemas.TestConfig, db: Session = Depends(get_db)):
    updated_test_config = update_test_config(db, test_config_id=test_config_id, test_config=test_config)
